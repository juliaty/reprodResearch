...
The mean of the total number of steps taken per-day is  `r as.character(round(meanVal,2))` and the median is `r medianVal`.
...
The 5-minute interval, on average across all the days in the dataset that contains the maximum number of steps is at interval`r maxdata$interval`.
...
The total number of rows with NAs is `r totNA`.
...
The mean of the total number of steps taken per-day is  `r as.character(round(meanVal,2))` and the median is `r as.character(round(medianVal,2))`.
